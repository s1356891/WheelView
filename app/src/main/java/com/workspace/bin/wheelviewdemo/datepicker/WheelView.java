...
package com.workspace.bin.wheelviewdemo.datepicker;
...
import com.workspace.bin.wheelviewdemo.R;
import com.workspace.bin.wheelviewdemo.datepicker.adapter.WheelViewAdapter;
 ...
 * @author enjoy
 * 功能优化：
 * 		需求：捕捉选中项，对选中项进行需要的设置
 * 		思路：得到选中项的index，适配器NumericWheelAdapter中对符合条件index的TextView设置
 * 		实现：当前选中的下标值currentItem 对象下标志
 * 			if(currentItem == resultItem || currentItem == index){
 * 				符合条件的下标值index  }
 * 		bug描叙：当currentItem=1时，会有多个对象被选中的状态
 * 		bug解决：if(currentItem == 1 && index == 0){
 * 				过滤该情况   }
 * 		bug描述：初始化执行buildViewForMeasuring()，手势滑动执行rebuildItems()方法
 * 				当手势滑动一个时，适配器只执行一个item，添加到显示的显示的items中。
 * 		bug解决：把rebuildItems()修改为buildViewForMeasuring()
 * 
 * 		需求：选中项显示清晰，未选中项模糊(使用遮罩层)
 * 		思路：选中项不使用遮罩层，未选中项使用遮罩层
 * 		实现：drawShadows()方法中计算上部(下部)的高度
 * 				int height = (int)( getItemHeight()*(visibleItems-1)/2);//两边显示的高度
 */
...
//			0xcfE9E9E9, 0x3fE9E9E9 };
	private int[] SHADOWS_COLORS = new int[] { 0xcfE9E9E9,
			 0xcfE9E9E9,  0xcfE9E9E9};
...
//			0x00AAAAAA, 0x00AAAAAA };
	...
	private void drawShadows(Canvas canvas) {
		...
		//*/
		//设置上部遮罩层
		topShadow.setBounds(0, 0, getWidth(), height-8);
		topShadow.draw(canvas);
		//设置下部遮罩层
		bottomShadow.setBounds(0, getHeight() - height+8, getWidth(), getHeight());
...
//		centerDrawable.draw(canvas);
		...
		paint.setColor(getResources().getColor(R.color.viewpage_listview_item_color_bg));
		// 设置线宽
